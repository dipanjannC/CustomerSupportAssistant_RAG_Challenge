...
@tracer.start_as_current_span(
    "RAG_with_conversation_memory", attributes={SpanAttributes.OPENINFERENCE_SPAN_KIND: "chain"})
def RAG_with_conversation_memory(query_text: str, retriever: Vectorstore) -> str:
    ...
    It retrieves relevant documents from a vectorstore based on the input query,
    and then generates a response using a language model with the retrieved context.
    The conversation memory is used to maintain the context of the conversation.
    ...
    try:
        #NOTE: Setting up conversation memory here might not be the best approach
        # As it runs recursively , and can lead to going into infinite loop
...
def CRAG(query_text: str, retriever: Vectorstore) -> str:
    """
    This function implements a Corrective Retrieval-Augmented Generation (CRAG) pipeline.
    It retrieves relevant documents from a vectorstore based on the input query,
    and then generates a response using a language model with the retrieved context.
...
if __name__ == "__main__":
    ...
    RAG_with_conversation_memory(query_text=query_text, retriever=vectorstore)
